...
from scraper.ub_metadata import DRIVER_PATH
from scraper.ub_metadata import IMG_SRC_CSS, HREF_CSS, PRODUCT_TITLE_CSS, PRICE_CSS, SELLER_CSS
...
def build_driver():
    global driver
    url = f'https://www.ubuy.com.bh/en/search/?q={keyword[count]}&page={pageList[-1]}'
...
def scroll_range(start_pixel, end_pixel):
    ...
    for pixel in range(start_pixel, end_pixel, 400):
...
def df_to_csv():
    ...
    df.to_csv(f"ubuy_{keyword}_{today}.csv", index=True, encoding='utf-8-sig')
    print("File Saved.")
